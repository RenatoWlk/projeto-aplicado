...
export class AccountService {
  ...
  getUserById(userId: string): Observable<UserResponseDTO> {
    return this.http.get<UserResponseDTO>(`/api/users/${userId}`)
      .pipe(catchError(this.handleError));
  }

  // Buscar estatísticas
  getUserStats(userId: string): Observable<UserStatsDTO> {
    return this.http.get<UserStatsDTO>(`/api/users/${userId}/stats`)
      .pipe(catchError(this.handleError));
  }
  ...
  getBloodBank(id: string): Observable<BloodBankUser> {
    return this.http.get<BloodBankUser>(`/api/bloodbanks/${id}`)
      .pipe(catchError(this.handleError));
  }

  createBloodBank(data: BloodBankRequestDTO): Observable<BloodBankUser> {
    return this.http.post<BloodBankUser>(`/api/bloodbanks`, data)
      .pipe(catchError(this.handleError));
  }

  // === CAMPAIGN METHODS ===
  createCampaign(bloodBankId: string, campaign: CampaignDTO): Observable<Campaign> {
    return this.http.post<Campaign>(`/api/bloodbanks/${bloodBankId}/campaigns`, campaign)
      .pipe(catchError(this.handleError));
  }

  // === PARTNER METHODS ===
  getPartner(id: string): Observable<PartnerUser> {
    return this.http.get<PartnerUser>(`/api/partners/${id}`)
      .pipe(catchError(this.handleError));
  }

  createPartner(data: PartnerRequestDTO): Observable<PartnerUser> {
    return this.http.post<PartnerUser>(`/api/partners`, data)
      .pipe(catchError(this.handleError));
  }

  // === OFFER METHODS ===
  createOffer(partnerId: string, offer: OfferDTO): Observable<Offer> {
    return this.http.post<Offer>(`/api/partners/${partnerId}/offers`, offer)
      .pipe(catchError(this.handleError));
  }
